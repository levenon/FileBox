...
static NSMutableDictionary * KXMovieViewControllerHistory;
...
@property (nonatomic, strong) KXArtworkFrame          *artworkFrame;
@end
...
+ (void)initialize{
    ...
    if (!KXMovieViewControllerHistory)
        KXMovieViewControllerHistory = [NSMutableDictionary dictionary];
}
...
+ (id)movieViewControllerWithContentPath: (NSString *)path
    ...
    [[KXAudioManager shareAudioManager] activateAudioSession];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    if (_decoder){
        ...
        if (_moviePosition == 0 || _decoder.isEOF)
            [KXMovieViewControllerHistory removeObjectForKey:_decoder.path];
        else if (!_decoder.isNetwork)
            [KXMovieViewControllerHistory setValue:[NSNumber numberWithFloat:_moviePosition]
...
- (void)restorePlay{
    ...
    NSNumber *n = [KXMovieViewControllerHistory valueForKey:_decoder.path];
    if (n)
...
- (void)enableAudio: (BOOL)on{
    ...
    id<KXAudioManager> audioManager = [KXAudioManager shareAudioManager];
...
- (void)updateHUD{
    ...
    NSString *subinfo = _decoder.validSubtitles ? [NSString stringWithFormat: @" %lu",_subtitles.count] : @"";
